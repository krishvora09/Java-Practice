...
public class BookingAgent {
    ...
    public static void main(String[] args) {
        ...
        int rows = 10;
        int totalSeats = 100;
        Theatre phoenixBLR = new Theatre("Phoenix Market City",
                rows, totalSeats);

        phoenixBLR.printSeatMap();

        bookSeat(phoenixBLR, 'A', 3);
        bookSeat(phoenixBLR, 'A', 3);

        bookSeat(phoenixBLR, 'B', 1);
        bookSeat(phoenixBLR, 'B', 11);
        bookSeat(phoenixBLR, 'M', 1);

        bookSeats(phoenixBLR, 4, 'B', 3, 10);
        bookSeats(phoenixBLR, 6, 'B', 'C', 3, 10);
        bookSeats(phoenixBLR, 4, 'B', 1, 10);
        bookSeats(phoenixBLR, 4, 'B', 'C', 1, 10);
        bookSeats(phoenixBLR, 1, 'B', 'C', 1, 10);
        bookSeats(phoenixBLR, 4, 'M', 'Z', 1, 10);
        bookSeats(phoenixBLR, 10, 'A', 'E', 1, 10);
    }
